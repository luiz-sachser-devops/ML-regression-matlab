...
path = ['Executions/Dataset2/',num2str(datetime().Year),'_',num2str(datetime().Month),'_',num2str(datetime().Day),'_',num2str(datetime().Hour),'_',num2str(datetime().Minute),'_',num2str(datetime().Second,2)];
mkdir(path);

path_data_file = [path,'/Dataset2Data.xls'];

%% Dataset Anallysis

dataToUse_opts = detectImportOptions('Milk.csv');
getvaropts(dataToUse_opts,{'Month','Production'});
dataToUse_opts.SelectedVariableNames = {'Month','Production'};

dataToUse_ds = readtable('Milk.csv',dataToUse_opts);
dataToUse_ds.Properties.VariableNames = {'Month','Production'};
dataToUse_ds.Month = datetime(dataToUse_ds{:,1}, 'InputFormat', 'yyyy-MM');
dataToUse_ds = rmmissing(dataToUse_ds);


figure, scatter(dataToUse_ds.Month,dataToUse_ds.Production), title('Scatter Close Value')
saveas(gcf,[path,'/Scatter Close Value.png']);

close all

% Normalize data
dataToUse_ds.ProductionNormalized = (dataToUse_ds.Production - min(dataToUse_ds.Production))/(max(dataToUse_ds.Production) - min(dataToUse_ds.Production));
pUnit = dataToUse_ds.Month;
features = datenum(dataToUse_ds.Month);
features = num2cell(features.');
targets = dataToUse_ds.Production;
targets = num2cell(targets.');

figure, plot(dataToUse_ds.Month,dataToUse_ds.ProductionNormalized), title('Scatter Production Value Normalized')
saveas(gcf,[path,'/scatter_Scatter Production Value Normalized.png']);
